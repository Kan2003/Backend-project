...
const deleteVideoFromCloud = async (fileUrl) => {
    if (!fileUrl) return null;

    // Extract publicId (accounting for the 'home' folder)
    const urlSegments = fileUrl.split('/');
    const fileNameWithExtension = urlSegments[urlSegments.length - 1];
    const publicIdWithoutExtension = fileNameWithExtension.split('.')[0];
    ...
    try {
        ...
        if (result.result === 'ok') {
            ...
            console.error(`Failed to delete video: ${result.result}`);
        }
...
export {uploadImage , deleteImage , uploadVideo , deleteVideoFromCloud}